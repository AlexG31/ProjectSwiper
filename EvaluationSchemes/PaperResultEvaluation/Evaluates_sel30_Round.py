...
if __name__ == '__main__':
    OutputFolder = os.path.join(curfolderpath,'100Round_Result')
    FN =  {
                'pos':[],
                'label':[],
                'recname':[]
    ...
    for round_i in xrange(1,101):
        RFfolder = r'F:\LabGit\ECG_RSWT\TestResult\pc\A_15\Round{}'.format(round_i)
        # =========================
        # 分析训练和测试的record数目
        # =========================
        # 计算误差
        curFN,curFP,curErr = EvalQTdbResults(getresultfilelist(RFfolder),OutputFolder)
    ...
    ecg_statistics.FP2CSV(FP,Err,os.path.join(OutputFolder,'sel30_100Rounds_FP.csv'))
    ecg_statistics.FN2CSV(FN,Err,os.path.join(OutputFolder,'sel30_100Rounds_FN.csv'))
    EvalLogfilename = os.path.join(OutputFolder,'Round100_plain_output.log')
    ECGstatistics.dispstat0(pFN = FN,pErr = Err,LogFileName = EvalLogfilename,LogText = 'Statistics of Results in FilePath [pc/R15]',OutputFolder = OutputFolder)
