...
from ECGPostProcessing.GroupResult_Score import ResultFilter
from sel30_ECGPloter import ECGResultPloter
...
def EvalQTdbResults(resultfilelist,OutputFolder,debug = False):
    ...
    for fi,fname in enumerate(resultfilelist):
        ...
        reslist = Results[1]
        # remove score
        #reslist = zip(*reslist)[0:2]
        #reslist = zip(reslist[0],reslist[1])
        ...
        if debug == True and len(pFN['pos']) > 0:
            PlotFN_inECG(QTdb,currecordname,pFN,fResults[0][1])
...
def PlotFN_inECG(QTdb,currecordname,pFN,reslist):
    sig_struct = QTdb.load(currecordname)
    expert_labels = QTdb.getexpertlabeltuple(currecordname)
    AdditionalPlot = []
    AdditionalPlot.append(('yx','expert labels',map(lambda x:x[0],expert_labels)))
    AdditionalPlot.append(('ks','False Negtives',pFN['pos']))
    ecg_ploter = ECGResultPloter(sig_struct['sig'],reslist)
    ecg_ploter.plot(AdditionalPlot = AdditionalPlot)
...
if __name__ == '__main__':
        ...
        # 计算误差
        OutputFolder = os.path.join(curfolderpath,'tmp')
        #os.mkdir(OutputFolder)
        EvalQTdbResults(getresultfilelist(RFfolder),OutputFolder,debug = False)
