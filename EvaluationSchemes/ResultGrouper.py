...
class ResultGrouper:
    ...
    '''
    def __init__(self,reslist,MaxSWTLevel = 9):
        self.recres = reslist
        self.result_list = reslist

        self.res_groups = None
        self.peak_dict = dict(T=[],P=[],Ponset = [],Poffset = [],Tonset = [],Toffset = [])
        #self.getSWTcoeflist(MaxLevel = MaxSWTLevel)

    def group_result(self,recres = None,white_del_thres = 20,cp_del_thres = 0):
        #
...
if __name__ == '__main__':
    ...
    for resfilepath in resfiles:
        ...
        with open(os.path.join(curfolderpath,'MultiLead2','GroupResult',
            recname+'.json'),'w') as fout:
