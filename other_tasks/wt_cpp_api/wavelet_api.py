...
def DTCWT_API(Signal, DecLevel, Wavelet_Remain):
    '''
    API for cpp functions.
    input: Python types.
           Call DTCWT function with matlabarray type.
    output:
           Return python list s_rec.
    '''
    if isinstance(Signal, list) == False:
        raise Exception('Signal input must be python list type!')
    Signal = matlabarray(Signal)
    Wavelet_Remain = matlabarray(Wavelet_Remain)
    s_rec = DTCWT(Signal, DecLevel, Wavelet_Remain)
    
    return np.array(s_rec).tolist()
