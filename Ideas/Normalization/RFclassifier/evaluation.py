...
class ECGstatistics:
    def __init__(self,fResultList,debug_showErrhist = False):
        # result list:
        ...
        self.debug_showErrhist = debug_showErrhist
        self.pErr = None
        self.pFN = None
...
#
    ...
    def pErr_analysis_to_log(\
        ...
        for label in labellist:
            ...
            numofErr[label] = cnt
            if self.debug_showErrhist == True:
    ...
    def dispstat0(self,pFN = None ,pErr = None,LogFileName = None,LogText = None):
        #
        # statistics
        #
        if pFN is None and self.pFN is None:
        ...
        for label in labellist:
            ...
            stats.append((self.arraymean(Errs),self.arraystdvar(Errs)))
        # Convert to ms
        #
        ...
        if LogFileName is not None:
            with open(LogFileName,'w') as fout:
                ...
                fout.write('\t'.join(map(\
                        ...
                        CSEtolerance))+'\n')
            ## number analysis
            pFNstat = self.pFN_analysis_to_log(LogFileName,labellist,pFN)
            pErrstat = self.pErr_analysis_to_log(LogFileName,labellist,pErr)
            # pFN percentage
            with open(LogFileName,'a') as fout:
                for label in labellist:
