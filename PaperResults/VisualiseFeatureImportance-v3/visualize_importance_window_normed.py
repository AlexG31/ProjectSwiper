...
class FeatureVisualizationSwt:
    ...
    def plot_dwt_pairs_arrow_transparent(self,
        ...
        for i in range(2, N_subplot):
            ...
            # -----------
            rel_layer = filter(lambda x:x[1] >= Thres_min_importance, rel_layer)
            ...
            for rel_pair, imp in zip(relation_pair_list, layer_importance_list):
                ...
                for temp_x in rel_x[-2:len(rel_x)]:
                    if (temp_x > window_limit_right or temp_x < window_limit_left):
                        out_of_bound = True
                        break
                if out_of_bound:
                    continue
