...
class PointBrowser(object):
    ...
    def __init__(self,fig,ax):
        ...
        self.resultlist = glob.glob(os.path.join('F:\LabGit\ECG_RSWT\TestResult\paper\MultiRound2\Round1','result_sel100'))
    ...
    def plotResultLabels(self,ax):
        ...
        for label,posList in labelDict.iteritems():
            ...
            if 'onset' in label:
                ...
                marker = '.'
            ax.plot(posList,map(lambda x:self.rawSig[x],posList),marker = marker,color = color,linestyle = 'none',markersize = 6,label = 'Pred[{}]'.format(label),alpha = 0.8)
