...
class GroupResult2Leads:
    ...
    def crop_data_for_swt(self):
        # crop self.rawsig
        base2 = 1
        N_data = len(self.rawsig)
        if len(self.rawsig)<=1:
            raise Exception('len(rawsig)={},not enough for swt!',len(self.rawsig))
        crop_len = base2
        while base2<N_data:
            if base2*2>=N_data:
                crop_len = base2*2
        ...
        # pad zeros
        if N_data< crop_len:
            self.rawsig += [self.rawsig[-1],]*(crop_len-N_data)
        self.rawsig = self.rawsig[0:crop_len]
    def getSWTcoeflist(self,MaxLevel = 9):
        self.crop_data_for_swt()
    ...
    def get_cross_zero_list(self,array):
        ...
        crosszerolist = []
        rem = array[0]
        for ind,val in enumerate(array[1:]):
            if rem<0 and val>=0:
                crosszerolist.append(ind)
            rem = val
        return crosszerolist
    ...
    def getResultDict(self,debug = False):
        ...
        for leadnum in xrange(1,3):
            ...
            for label in ['P','R','T','Ponset','Poffset','Toffset','Ronset','Roffset']:
                ...
                # result
                #print 'len of res groups:',len(self.res_groups)
                #with open(os.path.join(curfolderpath,'tmp.json'),'w') as fout:
                    #json.dump(self.res_groups,fout,indent = 4,sort_keys = True)
                #pdb.set_trace()
