...
if __name__ == '__main__':
        RFfolder = os.path.join(\
               ...
               'MultiRound1',\
               'Round*')
        ResultFolders = glob.glob(RFfolder)
        #getRRhisto()
        #sys.exit()
        ...
        # 计算误差
        for res_folder in ResultFolders:
            foldername = os.path.split(res_folder)[-1]
            OutputFolder = os.path.join(curfolderpath,'MultiLead1','eval_{}'.format(foldername))
            os.mkdir(OutputFolder)
            EvalQTdbResults(getresultfilelist(res_folder),OutputFolder)
