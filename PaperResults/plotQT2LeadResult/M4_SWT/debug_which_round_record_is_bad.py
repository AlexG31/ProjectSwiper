...
if __name__ == '__main__':
    ...
    with open(os.path.join(curfolderpath,'Log_which_record_is_bad.txt'),'w') as fin:
        pass
    # ===
    # Find the bad record with threhold on mean,std,FN,FP
    # ===
    police_obj = FindBadRecord(200, 200, 10, 10)
    # target label
    police_obj.possible_label_list = ['T',]

    for RoundInd in xrange(1,2):
        print 'processing Round :',RoundInd
        GroupResultFolder = os.path.join(curfolderpath,'TempResult','SWT_GroupRound_T_improved{}'.format(RoundInd))
