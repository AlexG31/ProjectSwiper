...
class PointBrowser(object):
    ...
    def __init__(self,fig,ax,ax2):
        ...
        self.resultlist = glob.glob(os.path.join(curfolderpath,'M4_SWT','SWT_GroupRound_T_improved{}'.format(RoundIndex),'*.json'))
        self.raw_resultlist = glob.glob(os.path.join('F:\LabGit\ECG_RSWT\TestResult\paper\MultiRound4\Round{}'.format(RoundIndex),'result_*'))


        # get target recname:
        target_index = GetRecordIndexbyName(self.resultlist,target_record_name)
    ...
    def PlotTpeakDetermineLines(self):
        ...
        for detail_ind in xrange(-5,-7,-1):
            ...
            ax.plot(e4list,color = self.colors[color_index],label = 'detail{}'.format(detail_ind))
            ax.legend()
            ...
            ax2.plot(e4list,color = self.colors[color_index],label = 'detail{}'.format(detail_ind))
            ax2.legend()
            # color change
            color_index += 1
        # 画WT的导数
